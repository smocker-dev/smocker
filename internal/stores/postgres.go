...
func (s *postgresStore) SaveCurrentTimeline(ctx context.Context, id *string) error {
...
func (s *postgresStore) LoadCurrentTimeline(ctx context.Context) (*string, error) {
	var id *string
...
func (s *postgresStore) SaveTimeline(ctx context.Context, id string, timeline types.Timeline) error {
	if _, err := s.db.ExecContext(ctx, `
...
func (s *postgresStore) LoadTimelines(ctx context.Context) (map[string]types.Timeline, error) {
...
func (s *postgresStore) DeleteTimeline(ctx context.Context, id string) error {
	if _, err := s.db.ExecContext(ctx, `
...
func (s *postgresStore) SaveMock(ctx context.Context, id uuid.UUID, timelineID string, position int, mock types.Mock) error {
	if _, err := s.db.ExecContext(ctx, `
...
func (s *postgresStore) LoadMocks(ctx context.Context) (map[string][]types.Mock, error) {
	rows, err := s.db.QueryContext(ctx, `
...
func (s *postgresStore) SaveMockTimesCalled(ctx context.Context, id uuid.UUID, timesCalled int) error {
...
func (s *postgresStore) LoadMocksTimesCalled(ctx context.Context) (map[string]map[uuid.UUID]int, error) {
	rows, err := s.db.QueryContext(ctx, `
...
func (s *postgresStore) DeleteMocksTimesCalled(ctx context.Context, timelineID string) error {
	if _, err := s.db.ExecContext(ctx, `
        UPDATE mocks
        SET times_called = 0
        WHERE timeline_id = $1
...
func (s *postgresStore) SaveHistoryEntry(ctx context.Context, id uuid.UUID, timelineID string, position int, history types.HistoryEntry) error {
	if _, err := s.db.ExecContext(ctx, `
...
func (s *postgresStore) LoadHistoryEntries(ctx context.Context) (map[string][]types.HistoryEntry, error) {
	rows, err := s.db.QueryContext(ctx, `
        SELECT timeline_id, data
        FROM history
        ORDER BY timeline_id, position
...
func (s *postgresStore) DeleteHistoryEntries(ctx context.Context, timelineID string) error {
	if _, err := s.db.ExecContext(ctx, `
        DELETE FROM history
        WHERE timeline_id = $1
    `, timelineID); err != nil {
